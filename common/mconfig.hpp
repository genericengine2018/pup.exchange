...
constexpr name   PROFIT_ACCOUNT = "puppynetwork"_n;
...
const uint64_t   MINING_ROUND_MAX = 1000000 * PUP_TIMES;
const uint64_t   MINING_ROUND_MAX_L = 100000 * PUP_TIMES;
const uint64_t   MINING_ROUND_MAX_U = 3000000 * PUP_TIMES;
...
constexpr double MINING_TOP_X_DEC_L = 0.4;
...
const uint64_t   MINING_VOL_LASTVOL_X_U = 20;

const uint64_t   MINING_STEP_LIMIT = 50;
const uint64_t   MINING_STEP_LIMIT_L = 10;
const uint64_t   MINING_STEP_LIMIT_U = 100;
...
struct mconfig {
    ...
    name        profit_account = PROFIT_ACCOUNT;
    ...
    uint64_t    step_limit = MINING_STEP_LIMIT;
...
class MConfHelper{
public:
    static mconfig Get(const name& contract,const name& scope){
        MConfig conf_table(contract,scope.value);   
        ...
        conf.step_limit = std::clamp(conf.step_limit,MINING_STEP_LIMIT_L,MINING_STEP_LIMIT_U);
        conf.fee_2_profit_p = std::clamp(conf.fee_2_profit_p,MINING_FEE_2_PROFIT_P_L,MINING_FEE_2_PROFIT_P_U);

        return conf;
    }

    static void Set(const mconfig& conf,const name& contract,const name& scope){
        MConfig conf_table(contract,scope.value);  
